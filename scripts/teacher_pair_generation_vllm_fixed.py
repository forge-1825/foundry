...
def verify_container_ports():
    ...
    try:
        ...
        logging.error("Docker command timed out while verifying container ports")
        return False
    except Exception as e:
    ...
    while time.time() - start_time < timeout:
        try:
            log_result = subprocess.run(['docker', 'logs', container], 
                                      capture_output=True, text=False, check=True, timeout=30)
            ...
            logging.error(f"Docker logs command timed out for container {container}")
            return False
...
def check_and_start_containers():
    ...
    try:
        ...
        # First check if Docker is running with timeout
        try:
            version_check = subprocess.run(['docker', 'version'], 
                                         capture_output=True, text=True, check=True, timeout=30)
            logging.info("Docker is running")
        except subprocess.TimeoutExpired:
            logging.error("Docker command timed out. Docker may be hanging or not responding.")
            logging.error("Please check Docker status using the diagnose_docker.py script.")
            return False
        except subprocess.CalledProcessError:
            logging.error("Docker is not running. Please start Docker first.")
            return False
        
        # Get list of running containers with timeout
        try:
            ...
            logging.error("Docker command timed out while listing containers.")
            logging.error("Please check Docker status using the diagnose_docker.py script.")
            return False
            ...
            for container in containers_to_start:
                ...
                try:
                    ...
                    try:
                        ...
                        logging.error(f"Docker command timed out while verifying container {container} is running")
                        return False
                except subprocess.TimeoutExpired:
                    logging.error(f"Docker command timed out while starting container {container}")
                    return False
                except subprocess.CalledProcessError as e:
...
def main():
    ...
    if not check_and_start_containers():
        ...
        logging.error("Run the diagnose_docker.py script to troubleshoot Docker issues.")
        sys.exit(1)
    ...
    domain_context = ""
    domain_context_path = "domain_context.txt"
    ...
    input_path = "enriched_data.json"
    output_path = "teacher_pairs_data.json"
