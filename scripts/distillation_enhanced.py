...
from memory_monitor import MemoryMonitor, log_memory_snapshot
...
memory_monitor = MemoryMonitor("distillation", interval_seconds=10.0)
...
try:
    ...
    log_memory_snapshot("distillation", "Before loading parent config")
    ...
    log_memory_snapshot("distillation", "After loading parent config")
    ...
    log_memory_snapshot("distillation", "Before loading tokenizer")
    tokenizer = AutoTokenizer.from_pretrained(student_model_path, local_files_only=True)
    log_memory_snapshot("distillation", "After loading tokenizer")
    ...
    if torch.cuda.is_available():
        ...
        log_memory_snapshot("distillation", "After clearing CUDA cache")
    ...
    log_memory_snapshot("distillation", "Before loading student model")
    ...
    log_memory_snapshot("distillation", "After loading student model")
    ...
    log_memory_snapshot("distillation", "Before loading teacher pairs")
    ...
    log_memory_snapshot("distillation", "After loading teacher pairs")
    ...
    log_memory_snapshot("distillation", "Before loading teacher model")
    ...
    log_memory_snapshot("distillation", "After loading teacher model")
    ...
    log_memory_snapshot("distillation", "Before creating datasets")
    ...
    log_memory_snapshot("distillation", "After creating datasets")
    ...
    log_memory_snapshot("distillation", "Before preparing model for k-bit training")
    student_model = prepare_model_for_kbit_training(student_model)
    log_memory_snapshot("distillation", "After preparing model for k-bit training")
    ...
    log_memory_snapshot("distillation", "Before applying LoRA")
    student_model = get_peft_model(student_model, lora_config)
    student_model.print_trainable_parameters()
    log_memory_snapshot("distillation", "After applying LoRA")

    # --------------------------------------------------------------------------
    # Initialize Accelerator with GPU optimizations and mixed precision.
    # --------------------------------------------------------------------------
    log_memory_snapshot("distillation", "Before initializing Accelerator")
    accelerator = Accelerator(
        gradient_accumulation_steps=32,
        mixed_precision="bf16" if torch.cuda.is_available() else None
    )
    log_memory_snapshot("distillation", "After initializing Accelerator")
    ...
    log_memory_snapshot("distillation", "Before preparing with Accelerator")
    ...
    log_memory_snapshot("distillation", "After preparing with Accelerator")
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            log_memory_snapshot("distillation", f"Start of epoch {epoch+1}")
            ...
            for step, batch in enumerate(train_dataloader):
                try:
                    ...
                    if completed_steps % 5 == 0:
                        ...
                        # Log memory usage
                        log_memory_snapshot("distillation", f"Step {completed_steps}")
            ...
            log_memory_snapshot("distillation", f"End of epoch {epoch+1}")
        ...
        log_memory_snapshot("distillation", "Before saving final model")
        accelerator.wait_for_everyone()
        unwrapped_model = accelerator.unwrap_model(student_model)
        try:
            unwrapped_model.save_pretrained(
        ...
        log_memory_snapshot("distillation", "After saving final model")
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        gc.collect()
        logging.info("Training resources cleaned up")
